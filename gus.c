...
/* DMA control register bits (from Linux kernel gus.h) */
#define DMA_ENABLE      0x01
#define DMA_READ        0x02  /* 0=read from GUS DRAM, 1=write to GUS DRAM */
#define DMA_WIDTH_16    0x04  /* 16-bit DMA channel */
#define DMA_RATE0       0x08
#define DMA_RATE1       0x10
#define DMA_IRQ_ENABLE  0x20  /* IRQ on DMA terminal count */
#define DMA_DATA_16     0x40  /* 16-bit PCM data */
#define DMA_INVERT_MSB  0x80  /* Invert MSB (unsigned/signed conversion) */
#define DMA_TC_IRQ      0x40  /* Read-back: bit 6 = DMA TC IRQ pending */
...
typedef struct GUSVoice {
    ...
    /* Ramp fractional accumulator (9-bit fraction for scale division) */
    uint16_t ramp_frac_accum;

    /* Real-time catch-up timestamps (for detection polling between audio callbacks) */
    uint32_t ramp_start_us;  /* Timestamp when ramp was last started/unblocked */
    uint32_t voice_start_us; /* Timestamp when voice playback was last started */
} GUSVoice;
...
struct GUSState {
    ...
    /* AdLib-compat real-time timer state (for OPL detection between audio callbacks) */
    uint8_t  adlib_timer1_count; /* OPL register 0x02 */
    uint8_t  adlib_timer2_count; /* OPL register 0x03 */
    uint32_t adlib_timer_start[2];  /* Start time in µs (get_uticks) */
    uint32_t adlib_timer_expire[2]; /* Period in µs */
    int      adlib_timer_ticking[2]; /* Actively counting */
    ...
    uint8_t  irq_latch_val;     /* Last value written to IRQ latch */
    uint8_t  dma_latch_val;     /* Last value written to DMA latch */
...
static uint8_t gus_irq_queue_pop(GUSState *s)
{
    if (s->irq_queue_count == 0) {
        /* Queue empty — clear wave/ramp IRQ status to deassert IRQ line.
         * Without this, a dropped push (queue full) leaves status set with
         * nothing to pop, causing the ISR to loop forever. */
        s->irq_status &= ~(IRQ_STAT_WAVE | IRQ_STAT_RAMP);
        gus_update_irq(s);
        return 0xE0; /* No IRQ pending: bits 7+6+5 set */
    }
...
static void gus_raise_wave_irq(GUSState *s, int voice)
{
    ...
    /* Register 0x8F format: bits 4:0 = voice, bit 5 = always set,
     * bit 6 = 0 if ramp IRQ pending (active low), bit 7 = 0 if wave IRQ pending (active low).
     * For wave IRQ: bit 7 clear, bit 6 set = 0x20 | 0x40 = 0x60 */
    gus_irq_queue_push(s, (voice & 0x1F) | 0x60);
    gus_update_irq(s);
}
...
static void gus_raise_ramp_irq(GUSState *s, int voice)
{
    ...
    /* For ramp IRQ: bit 6 clear, bit 7 set = 0x20 | 0x80 = 0xA0 */
    gus_irq_queue_push(s, (voice & 0x1F) | 0xA0);
    gus_update_irq(s);
}
...
static int gus_dma_handler(void *opaque, int nchan, int dma_pos, int dma_len)
{
    ...
    gus_log("DMA: ctrl=%02x addr=%06x len=%d pos=%d\n", s->dma_control, gus_addr, dma_len, dma_pos);

    int data_16 = (s->dma_control & DMA_DATA_16) ? 1 : 0;
    /* 16-bit DMA with 16-bit data needs address translation (bank interleave) */
    int xlate_addr = (s->dma_control & DMA_WIDTH_16) && data_16;
    ...
    while (remaining > 0) {
        ...
        /* Copy into GUS RAM, applying address translation and data conversion */
        for (int i = 0; i < copied; i++) {
            uint8_t byte = buf[i];
            /* Invert MSB: for 8-bit data, XOR every byte;
             * for 16-bit data, XOR only the high byte of each word */
            if (invert_msb) {
                if (!data_16)
                    byte ^= 0x80;
                else if (i & 1)  /* odd byte = high byte of 16-bit word */
                    byte ^= 0x80;
            }
            uint32_t dest = gus_addr;
            if (xlate_addr)
                dest = gus_addr_16bit(gus_addr >> 1) + (gus_addr & 1);
            s->gus_ram[dest & GUS_RAM_MASK] = byte;
            gus_addr++;
        }
...
static void gus_voice_boundary(GUSState *s, GUSVoice *v, int voice_num,
                               uint32_t overshoot)
{
    if (v->voice_ctrl & VC_LOOP) {
        if (v->voice_ctrl & VC_BIDIR) {
            /* Bidirectional: reverse direction, preserve overshoot */
            v->voice_ctrl ^= VC_BACKWARD;
            if (v->voice_ctrl & VC_BACKWARD)
                v->current_addr = v->end_addr - overshoot;
            else
                v->current_addr = v->start_addr + overshoot;
        } else {
            /* Forward/backward loop: wrap with overshoot */
            if (v->voice_ctrl & VC_BACKWARD)
                v->current_addr = v->end_addr - overshoot;
            else
                v->current_addr = v->start_addr + overshoot;
        ...
        /* No loop: stop voice, snap to boundary */
...
static inline void gus_advance_voice(GUSState *s, GUSVoice *v, int voice_num)
{
    ...
    if (v->voice_ctrl & VC_BACKWARD) {
        if (v->current_addr < v->start_addr + step) {
            uint32_t overshoot = v->start_addr + step - v->current_addr;
            gus_voice_boundary(s, v, voice_num, overshoot);
        } else {
            v->current_addr -= step;
        }
    } else {
        ...
        if (v->current_addr >= v->end_addr) {
            uint32_t overshoot = v->current_addr - v->end_addr;
            gus_voice_boundary(s, v, voice_num, overshoot);
        }
    }
}
...
static inline void gus_advance_ramp(GUSState *s, GUSVoice *v, int voice_num)
{
    ...
    /* Rate register: bits 5:0 = increment, bits 7:6 = scale (divide by 8^scale).
     * Higher scale = slower ramp. Use 9-bit fractional accumulator so
     * divided steps don't lose precision. */
    int increment = v->ramp_rate & 0x3F;
    int scale = (v->ramp_rate >> 6) & 0x03;
    /* Fractional step: shift left by (9 - 3*scale) to effectively divide by 8^scale */
    static const int scale_shift[] = {9, 6, 3, 0};
    uint32_t step_frac = (uint32_t)increment << scale_shift[scale];

    v->ramp_frac_accum += step_frac;
    int step = v->ramp_frac_accum >> 9;
    v->ramp_frac_accum &= 0x1FF;
...
/* Catch up voice playback position based on elapsed real time.
 * Called when register 0x80 is polled, so detection code that starts a voice
 * and polls for VC_STOPPED sees it complete without waiting for audio callback. */
static void gus_catchup_voice(GUSState *s, GUSVoice *v, int voice_num)
{
    if (v->voice_ctrl & (VC_STOPPED | VC_STOP))
        return;

    uint32_t now = get_uticks();
    uint32_t elapsed_us = now - v->voice_start_us;
    if (elapsed_us == 0)
        return;
    ...
    uint32_t ticks = (uint64_t)elapsed_us * internal_rate / 1000000;
    if (ticks > 65536) ticks = 65536;

    if (ticks == 0)
        return; /* Not enough time for even 1 tick — let it accumulate */

    for (uint32_t i = 0; i < ticks; i++) {
        gus_advance_voice(s, v, voice_num);
        if (v->voice_ctrl & (VC_STOPPED | VC_STOP))
            break;
    }

    v->voice_start_us = now;
}

/* Catch up volume ramp based on elapsed real time.
 * Called when register 0x8D is polled, so detection code that starts a ramp
 * and polls for RC_STOPPED sees it complete without waiting for audio callback. */
static void gus_catchup_ramp(GUSState *s, GUSVoice *v, int voice_num)
{
    if (v->ramp_ctrl & (RC_STOPPED | RC_STOP))
        return;

    uint32_t now = get_uticks();
    uint32_t elapsed_us = now - v->ramp_start_us;
    if (elapsed_us == 0)
        return;

    /* GF1 internal rate: GF1_CLOCK / (active_voices * 16) */
    int nv = s->active_voices;
    if (nv < 14) nv = 14;
    if (nv > 32) nv = 32;
    uint32_t internal_rate = GF1_CLOCK / (nv * 16);

    /* How many ramp ticks should have elapsed */
    uint32_t ticks = (uint64_t)elapsed_us * internal_rate / 1000000;
    if (ticks > 65536) ticks = 65536; /* Safety cap */

    if (ticks == 0)
        return; /* Not enough time for even 1 tick — let it accumulate */

    for (uint32_t i = 0; i < ticks; i++) {
        gus_advance_ramp(s, v, voice_num);
        if (v->ramp_ctrl & (RC_STOPPED | RC_STOP))
            break;
    }

    /* Update timestamp so next poll only advances by the new delta */
    v->ramp_start_us = now;
}
...
void gus_audio_callback(void *opaque, uint8_t *stream, int free)
{
    ...
    /* Periodic GUS state dump (~1/sec) */
    {
        ...
        if (++cb_count >= 44100) {
            ...
            int running = 0;
            for (int v = 0; v < 32; v++)
                if (!(s->voices[v].voice_ctrl & (VC_STOPPED | VC_STOP)))
                    running++;
            gus_log("audio_cb: reset=%02x voices_running=%d active=%d\n",
                    s->reset_reg, running, s->active_voices);
        }
    }
    ...
    /* Update catch-up timestamps so register polling doesn't double-count
     * ticks already processed by the audio callback */
    uint32_t cb_now = get_uticks();
    for (int v = 0; v < nv; v++) {
        GUSVoice *voice = &s->voices[v];
        if (!(voice->voice_ctrl & (VC_STOPPED | VC_STOP)))
            voice->voice_start_us = cb_now;
        if (!(voice->ramp_ctrl & (RC_STOPPED | RC_STOP)))
            voice->ramp_start_us = cb_now;
    }
}
...
/* Split a 32-bit accumulator address into high 16-bit register value.
 * Accumulator format: {high_word[15:0], low_word[15:0]} where
 * bits 31:9 = 23-bit sample address, bits 8:0 = 9-bit fraction. */
static inline uint16_t gus_addr_high(uint32_t addr)
{
    return (uint16_t)(addr >> 16);
}

/* Split a 32-bit accumulator address into low 16-bit register value */
...
static void gus_reg_write16(GUSState *s, uint8_t reg, uint16_t val)
{
    ...
    case 0x02: /* Start address high */
        v->start_addr = (v->start_addr & 0xFFFF) | ((uint32_t)val << 16);
        break;
    case 0x03: /* Start address low */
        v->start_addr = (v->start_addr & 0xFFFF0000) | val;
        break;
    case 0x04: /* End address high */
        v->end_addr = (v->end_addr & 0xFFFF) | ((uint32_t)val << 16);
        break;
    case 0x05: /* End address low */
        v->end_addr = (v->end_addr & 0xFFFF0000) | val;
        break;
    case 0x09: /* Current volume */
        v->current_vol = val;
        break;
    case 0x0A: /* Current address high */
        v->current_addr = (v->current_addr & 0xFFFF) | ((uint32_t)val << 16);
        break;
    case 0x0B: /* Current address low */
        v->current_addr = (v->current_addr & 0xFFFF0000) | val;
        break;
...
static void gus_reg_write8(GUSState *s, uint8_t reg, uint8_t val)
{
    ...
    case 0x00: /* Voice control */
        ...
        /* Record timestamp when voice is started (stop bits cleared) */
        if (!(val & (VC_STOPPED | VC_STOP))) {
            v->voice_start_us = get_uticks();
            gus_log("voice %d START: ctrl=%02x freq=%04x addr=%05x-%05x vol=%04x\n",
                    s->current_voice & 0x1F, val, v->freq_ctrl,
                    v->start_addr >> 9, v->end_addr >> 9, v->current_vol);
        }
        break;
    ...
    case 0x0D: /* Volume ramp control */
        ...
        /* Record timestamp when ramp is started (stop bits cleared) */
        if (!(val & (RC_STOPPED | RC_STOP))) {
            v->ramp_start_us = get_uticks();
            v->ramp_frac_accum = 0;
        }
        break;
    case 0x0E: /* Active voice count (write: set to N-1, range 13..31) */
        s->active_voices = (val & 0x1F) + 1;
    ...
    case 0x41: /* DMA control */
        ...
        break;
    case 0x43: /* DRAM address low (8-bit access sets upper byte) */
        s->dram_addr = (s->dram_addr & 0xF00FF) | ((uint32_t)val << 8);
        break;
    ...
    case 0x4C: /* Reset register */
        {
            ...
            /* When transitioning from running to reset (bit 0: 1→0),
             * bits 1-2 (DAC enable, master IRQ) are auto-cleared */
            if ((old & RESET_RUN) && !(val & RESET_RUN)) {
                s->reset_reg = val & RESET_RUN;
            } else {
                s->reset_reg = val & 0x07;
            }
            gus_log("reset_reg: %02x -> %02x (wrote %02x)\n", old, s->reset_reg, val);
...
static uint8_t gus_reg_read8(GUSState *s, uint8_t reg)
{
    ...
    case 0x80: /* Voice control (read-back) */
        /* Catch up voice position so polling detection sees voice stop */
        gus_catchup_voice(s, v, s->current_voice & 0x1F);
        return v->voice_ctrl;
    ...
    case 0x8D: /* Volume ramp control (read-back) */
        /* Catch up ramp based on elapsed real time so polling detection
         * sees the ramp complete without waiting for audio callback */
        gus_catchup_ramp(s, v, s->current_voice & 0x1F);
        return v->ramp_ctrl;
    case 0x8E: /* Active voice count (read-back) */
        return (s->active_voices - 1) & 0x1F;
 ...
 * ---------------------------------------------------------------- */
static uint8_t adlib_reg_select;    /* Latched AdLib register address (for 0x389 dispatch) */
static uint8_t adlib_commandreg;    /* Last byte written to port base+0x08, read back from 0x0A */
...
uint32_t gus_read(void *opaque, uint32_t port)
{
    ...
    case 0x08: /* Timer control (AdLib-compatible status) */
        {
            /* Fire expired timers before returning status */
            uint32_t now = get_uticks();
            for (int n = 0; n < 2; n++) {
                if (s->adlib_timer_ticking[n]) {
                    uint32_t elapsed = now - s->adlib_timer_start[n];
                    if (elapsed >= s->adlib_timer_expire[n]) {
                        if (n == 0 && !(s->adlib_timer_ctrl & 0x40))
                            s->irq_status |= IRQ_STAT_TIMER1;
                        if (n == 1 && !(s->adlib_timer_ctrl & 0x20))
                            s->irq_status |= IRQ_STAT_TIMER2;
                        s->adlib_timer_start[n] = now;
                    }
                }
            }
    ...
    case 0x0A: /* AdLib command register read-back */
        return adlib_commandreg;
    case 0x0B: /* IRQ/DMA latch read-back */
        {
            uint8_t v = s->irq_latch_select ? s->irq_latch_val : s->dma_latch_val;
            return v;
        }
    case 0x0C: /* MIDI receive data (stub - no MIDI hardware) */
        return 0x00;
    case 0x0E: /* MIDI status (stub - no MIDI hardware, return "no data") */
        return 0x00;
...
void gus_write(void *opaque, uint32_t port, uint32_t val)
{
    ...
    case 0x00: /* Mix control register */
        ...
        break;
    case 0x08: /* AdLib command register select */
    case 0x0A: /* AdLib command register select (alternate port) */
    ...
    case 0x09: /* Timer data (AdLib-compatible) */
        if (val & 0x80) {
            ...
            s->adlib_timer_ticking[0] = 0;
            s->adlib_timer_ticking[1] = 0;
            gus_update_irq(s);
            break;
        }
        ...
        /* Start/stop timer 1 */
        if (val & 0x01) {
            if (!s->adlib_timer_ticking[0]) {
                s->adlib_timer_ticking[0] = 1;
                s->adlib_timer_start[0] = get_uticks();
                s->adlib_timer_expire[0] = (256 - s->adlib_timer1_count) * 80;
            }
            s->timer1_running = 1;
        } else {
            s->timer1_running = 0;
        ...
        /* Start/stop timer 2 */
        if (val & 0x02) {
            if (!s->adlib_timer_ticking[1]) {
                s->adlib_timer_ticking[1] = 1;
                s->adlib_timer_start[1] = get_uticks();
                s->adlib_timer_expire[1] = (256 - s->adlib_timer2_count) * 320;
            }
            s->timer2_running = 1;
        } else {
            s->timer2_running = 0;
    ...
    case 0x0B: /* IRQ/DMA control register (latched by mix control bit 6) */
        ...
        break;
    case 0x06: /* IRQ status (read-only on real HW, writes ignored) */
        break;
...
uint32_t gus_read_gf1(void *opaque, uint32_t port)
{
    ...
    case 0x06: /* GF1 IRQ status (same as base+0x06) */
        return s->irq_status;
    case 0x07: /* DRAM I/O - read byte from GUS RAM */
        {
            uint8_t v = s->gus_ram[s->dram_addr & GUS_RAM_MASK];
            /* gus_log("DRAM read [%05x] = %02x\n", s->dram_addr & GUS_RAM_MASK, v); */
            return v;
        }
    default:
        gus_log("gf1 read unhandled offset %02x\n", offset);
        return 0xFF;
    }
}
...
void gus_write_gf1(void *opaque, uint32_t port, uint32_t val)
{
    ...
    case 0x07: /* DRAM I/O - write byte to GUS RAM */
        /* gus_log("DRAM write [%05x] = %02x\n", s->dram_addr & GUS_RAM_MASK, val); */
 ...
 * ---------------------------------------------------------------- */
/* (moved to before gus_read/gus_write) */

uint32_t gus_adlib_read(void *opaque, uint32_t port)
{
    GUSState *s = opaque;

    /* Check and fire expired AdLib timers in real-time (same approach as adlib.c) */
    uint32_t now = get_uticks();
    for (int n = 0; n < 2; n++) {
        if (s->adlib_timer_ticking[n]) {
            uint32_t elapsed = now - s->adlib_timer_start[n];
            if (elapsed >= s->adlib_timer_expire[n]) {
                /* Timer expired — set reached flag if not masked */
                if (n == 0 && !(s->adlib_timer_ctrl & 0x40))
                    s->irq_status |= IRQ_STAT_TIMER1;
                if (n == 1 && !(s->adlib_timer_ctrl & 0x20))
                    s->irq_status |= IRQ_STAT_TIMER2;
...
void gus_adlib_write(void *opaque, uint32_t port, uint32_t val)
{
    ...
    if ((port & 0xF) == 0x8) {
        ...
        switch (adlib_reg_select) {
        case 0x02: /* Timer 1 count */
            s->adlib_timer1_count = val;
            /* Also update GF1 timer count (shared on real hardware) */
            s->timer1_count = val;
            break;
        case 0x03: /* Timer 2 count */
            s->adlib_timer2_count = val;
            s->timer2_count = val;
            break;
        case 0x04: /* Timer control */
            ...
            /* Timer 1 masked flag: bit 6 */
            /* Timer 2 masked flag: bit 5 */
            /* Start/stop timer 1: bit 0 */
            if (val & 0x01) {
                if (!s->adlib_timer_ticking[0]) {
                    s->adlib_timer_ticking[0] = 1;
                    s->adlib_timer_start[0] = get_uticks();
