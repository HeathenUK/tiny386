...
static void i386_task(void *arg)
{
	...
	// Initialize USB host subsystem (non-blocking — tasks enumerate in background)
	extern esp_err_t usb_host_init(void);
	esp_err_t usb_err = usb_host_init();
	if (usb_err != ESP_OK) {
		fprintf(stderr, "USB host init failed: %d\n", usb_err);
	} else {
		fprintf(stderr, "USB: Host init OK, enumeration runs in background\n");
	}
	...
	/* Wait for SD card to be mounted (storage_init runs in parallel on core 0) */
	while (!storage_sd_mounted())
...
/* Runs storage_init + i2s_main on core 0 at priority 0, so vga_task
 * (same core/priority) can time-slice and display the splash screen. */
static void storage_audio_init_task(void *arg)
{
	(void)arg;
	storage_init();  /* SDMMC native slot 0 — ~1.5s SD power cycle */
	i2s_main();      /* Audio task on core 0 — after SDMMC init */
...
void app_main(void)
{
	...
	/* Start display + CPU tasks so vga_task shows splash during SD mount */
	...
	/* SD + audio init in a separate task at priority 0 so vga_task (also
	 * priority 0 on core 0) can time-slice and show the splash screen
	 * during the ~1.5s SD power cycle.  app_main returns immediately,
	 * freeing its priority-1 slot on core 0. */
	xTaskCreatePinnedToCore(storage_audio_init_task, "sd_init", 4096, NULL, 0, NULL, 0);
}
